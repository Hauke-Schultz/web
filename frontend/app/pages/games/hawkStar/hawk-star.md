...
- **Both meters are off** (`:battery="false" :shield="false"`). A blackout is already an alarm row and therefore already inside the badge — drawing it again as a pulsing ring would be two alarms for one fact — and the shield has its own bar with a figure on the card the disc scrolls to. One number per disc is what makes the band scannable.
...
- **`battery` and `shield` can each be switched off.** Both default true, and the **orbit map is the only caller that leaves them on** — there a marker is alone with its planet, and the ring and the aura are the only place its charge is written down. Both strips turn both off: the planet-grid strip prints the two figures in its chip line, and the Empire band puts one number per disc (its notice badge, which a blackout is already inside) and leaves the shield to the card the disc scrolls to.
...
- **The airlock is the readout** *(2026-09-03)*. The lock's two doors **shut whenever the approach is inside the allowance and spring open the moment it is not**, so *would a touchdown right now be held?* is answered by a mechanism rather than by a rim colour: slow enough and there is a floor to set down on, too fast and there is a hole. A landing puts the hulk **on** the closed doors — `y = 100` is the hatch, not the bottom edge of the box — which is why the hulk is drawn belly-down on the line (`translate(-100%)`, with the thruster flame taken out of flow so lighting it no longer shoves the ship up a few pixels) and why `COLLAR_PCT` is written from one place into both the band's height and the touchdown line. Two copies of that number drift apart on the first tweak and leave the hulk hovering over the hatch or sunk into it. A miss keeps going: the physics has stopped by then, so a CSS transition carries it through the gap and out of the field. `hot` and `dry` fall through; **`adrift` does not** — it never reached the doors and has to stay where it stalled. The outcome strings moved with the picture, since nothing bounces off anything any more.
- **The field scrolls itself into the middle of the viewport as it mounts** *(2026-09-03)*. The approach opens from a button at the bottom of a card that is itself well down a scrolled page, so on a phone the shaft came up mostly below the fold: the hulk entered at the top of something the player could not see, and the first thing they got to look at was the collar it was already falling toward. *When* to burn is the entire game and it cannot be judged through a letterbox. `scrollIntoView({ block: 'center' })` one frame after the next tick — `center` rather than the `nearest` used elsewhere, because nearest stops with a field this tall flush against an edge. Scrolling unprompted is safe here precisely because nothing is falling yet: the hulk waits in `ready` until the player releases a press on the field.
...
The three parallel unit rows collapsed into `targetOps` — every researched unit class contributes at most one row, and only inside the open planet's row.

Everything flies **from the active planet**, and that used to be spelled out above the rows as a `Start: <planet>` line. It has since been removed: sitting inside the row you just opened, it read like a statement about *that* planet, while the 📍 flag in the list already marks the origin and the dispatch column measures its flight times from it. Three statements of one fact, one of them in the wrong place.

### The op row names the origin, and lost its progress bar  *(2026-08-26)*

An op row said *what* is inbound and *in how long*, never *from where* — and a cargo run can leave from any colony, so the countdown alone did not identify the flight. The server has always sent `fromPlanetId`; the client was dropping it when mapping missions into each planet's dock. Carried through now, and the row reads `📦 Cargo Drone · von Vega IV · 1h 58m`.

The **progress bar came out**. It animated `hs-bar-fill` from `scaleX(0)` with nothing but `animationDuration`, so it only ever told the truth on a bar mounted at the exact moment of launch: after a reload, a flight two hours in drew an empty bar. The row already carries the origin and the countdown, which is the whole story, so the bar was dropped rather than repaired.

Repairing it was possible, though, and the same defect **was** live elsewhere — which is the part worth keeping in mind. Elapsed time is derivable (total flight time minus what is left), and `HsDockPanel`'s mission bars used the same helpers against CSS that carried *no keyframes at all*, so there the duration was ignored and every bar sat permanently full. The four helpers (`droneProgressStyle`, `colonyProgressStyle`, `cargoProgressStyle`, `cargoReturnProgressStyle`) now go through `flightProgressStyle(endsAt, totalSec)` and return a **width percentage driven by `now`** — the form the build bars have always used. A bar built from a duration alone cannot survive a reload; one built from a percentage does not care when it was mounted.
...
Picking a unit in the **hangar strip** now arms it. `armedUnit` (which also drives that unit's build accordion, so the two can never disagree) puts the list into dispatch mode: a **column slides open on the left of every row**, the rows make room for it, and each planet the unit can reach gets a send button carrying its flight time — collapsed rows included, so the whole system is one glance.

- **The column opens on every row, not only the reachable ones.** A planet the unit cannot reach keeps the space and stays empty; that is what holds the buttons in one column you can run your eye down, instead of a ragged edge. A target that is reachable but has nothing in the dock shows a muted `–` with the reason on hover — no button, but still distinguishable from "not a target at all".
- **The origin row carries the unit icon, not a blank.** Every flight time in the column is measured from the active planet, so leaving the one row that explains the others empty was the wrong kind of silence. It shows the armed unit's icon and an outgoing `→` in a **dashed** pill — dashed so it never reads as a button you failed to press: that row is where the unit stands, not somewhere it can go.
- **The width animates from 0 rather than mounting.** The column is always in the DOM and `--on` gives it a width; a `v-if` would pop in with no transition and the rows would jump instead of making room.
- **The column sits outside `hs-plist__row`**, which is itself a `<button>` — a send button nested in it would be invalid HTML and unclickable. Both live in a `hs-plist__head` flex row, and the body still spans the full width below.
...
- **A flight in progress is a state, not an absence.** Every `isXTarget` goes false while one is running — one flight per class at a time — so without an `enRoute` branch the column would go blank at exactly the moment there is something to report. The target's cell counts the mission down instead (`📦 1h 58m`), in the same footprint as the send button so the column does not twitch when one becomes the other, but flat and uninteractive: there is nothing left to press, only something to wait for.
...
**A unit that is away stops being a count.** `0` in the dock is true and useless — it looks exactly like never having built one. The strip button shows the leg and its clock instead (`→ 1h 58m` outbound, `← 42m` home), dashed like the origin marker in the list: the unit belongs to this planet, it is just not standing here right now. `unitAway()` reads the **active planet's** own missions (`activeDroneMissions` and friends, not the `allActive*` variants), so the strip only ever reports flights that left from here. The cargo drone is the only one with a return leg, which is why it is the only one that can show `←` — and that retired the separate `Rückflug` line in the hangar head, which was the same clock twice.
...
| Client | `reconDroneLevel` / `colonyShipLevel` / `spyDroneLevel` return **0 off the home planet**. They read as *"what can this planet produce"*, not *"what is built here"* — one gate reaches the build UI, the send buttons and the mission targeting at once. `canBuildCorvette` states it directly, since the fleet has no level computed of its own. **Espionage is where sending was split back off** — see the Espionage rules: its buttons live on the galaxy map, which is not about the active planet at all. |
...
- **Sending follows building** — for recon and colonisation. Because the capability computeds are the gate, a colony offers no launches of either; those flights start at home. A unit parked at a colony from before this change has no send button; there is no migration for it. **Espionage is the exception** *(2026-08-31)*: it launches from home too, but its buttons are offered from any active planet, because they sit on the galaxy map. See the Espionage rules.
...
- **The launcher is always home, whichever planet is on screen** *(2026-08-31)*. The galaxy map is one map: which of your own colonies happens to be active says nothing about a foreign system, so it must not decide whether the 🕵️ and 📡 buttons exist. Every spy unit sits in the home dock anyway (the hangar is `homeOnly`), the flight time is already measured from the **home system**, and the server bills the planet it is handed — so espionage reads hangar level, stock and the one-flight rule off `spyLaunchPlanetId` (home) rather than the active planet, and `sendSpyUnit` defaults `fromPlanetId` to it. Before this, switching to a colony silently emptied the galaxy card of every send button, which reads as a broken map rather than a rule. **Building** still follows the active planet, because building really is home-only.
